    ...
    if args_dict == None:
        ...
        if pydoop.hdfs.path.exists(path_to_metric):
            with pydoop.hdfs.open(path_to_metric, "r") as fi:
                ...
                return metric, hopshdfs._get_experiments_dir() + '/' + app_id + '/random_search/run.' +  str(run_id)

    return None, hopshdfs._get_experiments_dir() + '/' + app_id + '/random_search/run.' +  str(run_id)
